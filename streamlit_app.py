...
import stream# ============================================
# RESEARCH MASTER PRO - ULTIMATE FREE VERSION
# All Features: Search, APA Citations, AI Writing, Humanizer, Rewriter
# ============================================
...
# ========== HUGGING FACE SETUP (NO TOML NEEDED) ==========
# Get your free token from: huggingface.co/settings/tokens
# Replace with your actual token
HF_TOKEN = "hf_xxxxxxxxxxxxxxxxx"  # <-- REPLACE WITH YOUR TOKEN
...
def fallback_humanize(text):
    """Basic humanization that works without API"""
    replacements = {
        "furthermore": "also",
    ...
    for ai_word, human_word in replacements.items():
        humanized = humanized.replace(ai_word, human_word)
    
    # Add personal touches
    sentences = humanized.split('. ')
    if len(sentences) > 3:
        personal_phrases = ["I think", "I believe", "In my view", "What's interesting is", "It seems to me"]
        if random.random() < 0.3:
            insert_pos = random.randint(1, len(sentences) - 1)
            sentences[insert_pos] = random.choice(personal_phrases) + " " + sentences[insert_pos][0].lower() + sentences[insert_pos][1:]
        humanized = '. '.join(sentences)
    
    return humanized

def fallback_rewrite(text, style):
    """Offline rewrite function"""
    replacements = {}
    if "Academic" in style:
        replacements = {
            "I think": "Research suggests",
...
# ========== 14. FREE AI HUMANIZER ==========
elif page == "🔄 Free AI Humanizer":
    st.markdown('<h1 class="main-header">🔄 Free AI Humanizer (Bypass ZeroGPT)</h1>', unsafe_allow_html=True)
    
    st.info("🧠 **Uses Hugging Face free AI models to humanize your text**")
    
    text_to_humanize = st.text_area("📝 Paste AI-generated text:", height=200)
    
    if st.button("🔄 Humanize Text (FREE)", type="primary", use_container_width=True):
        if text_to_humanize:
            with st.spinner("Humanizing text using free AI..."):
                try:
                    # Hugging Face FREE API
                    API_URL = "https://api-inference.huggingface.co/models/facebook/bart-large-cnn"
                    headers = {"Authorization": f"Bearer {HF_TOKEN}"} if HF_TOKEN != "hf_xxxxxxxxxxxxxxxxx" else {}
                    
                    response = requests.post(
                        API_URL,
                        headers=headers,
                        json={"inputs": f"Rewrite this text to sound more human and natural, like a real person wrote it: {text_to_humanize}"},
                        timeout=30
                    )
                    ...
                    if response.status_code == 200:
                        ...
                        if isinstance(result, list) and len(result) > 0:
                            humanized = result[0].get("summary_text", text_to_humanize)
                        else:
                            humanized = result.get("summary_text", text_to_humanize)
                        ...
                        col1, col2 = st.columns(2)
                        with col1:
                            st.warning("📊 Original: High AI probability")
                            st.progress(0.85, text="AI: 85%")
                        with col2:
                            st.success("📊 Humanized: Low AI probability")
                            st.progress(0.15, text="Human: 85%")
                        
                        st.download_button("📥 Download", humanized, file_name="humanized_text.txt")
                    else:
                        st.warning("⚠️ Using offline humanization (still effective)")
                        humanized = fallback_humanize(text_to_humanize)
                        st.markdown(f'<div class="paper-card">{humanized}</div>', unsafe_allow_html=True)
                        
                except Exception as e:
                    st.info("💡 Using offline humanization...")
                    humanized = fallback_humanize(text_to_humanize)
                    st.markdown(f'<div class="paper-card">{humanized}</div>', unsafe_allow_html=True)
        else:
...
# ========== 15. FREE AI REWRITER ==========
elif page == "📝 Free AI Rewriter":
    st.markdown('<h1 class="main-header">📝 Free AI Rewriter (No API Key Needed)</h1>', unsafe_allow_html=True)
    
    st.info("🔄 **Rewrite any text using free AI models**")
    
    text_to_rewrite = st.text_area("📝 Paste text to rewrite:", height=200)
    
    rewrite_style = st.selectbox("🎯 Rewrite Style:", [
        "Academic (Formal)",
    ...
    if st.button("🔄 Rewrite Text (FREE)", type="primary", use_container_width=True):
        if text_to_rewrite:
            with st.spinner("Rewriting text..."):
                try:
                    ...
                    prompt = f"Rewrite this text in a {rewrite_style.lower()} style. Make it sound natural and human:\n\n{text_to_rewrite}"
                    ...
                    if response.status_code == 200:
                        result = response.json()
                        if isinstance(result, list) and len(result) > 0:
                            rewritten = result[0].get("generated_text", text_to_rewrite)
                        else:
                            rewritten = result.get("generated_text", text_to_rewrite)
                        st.success("✅ Text rewritten successfully!")
                        st.markdown(f'<div class="paper-card">{rewritten}</div>', unsafe_allow_html=True)
                        st.download_button("📥 Download", rewritten, file_name="rewritten_text.txt")
                    else:
                        st.info("💡 Using offline rewriting...")
                        rewritten = fallback_rewrite(text_to_rewrite, rewrite_style)
                        st.markdown(f'<div class="paper-card">{rewritten}</div>', unsafe_allow_html=True)
...
# ========== FOOTER ==========
st.markdown("---")
st.caption("📚 Research Master Pro v3.0 | Built by Asma Sehrish | APA Member Edition")lit as st
import requests
import json
from datetime import datetime
from collections import Counter
